...
public:
    ...
    void remove_node(int value){
        ...
        // determine if its just a leaf
            // delete it
        // if the value being deleted only has one child
            // then its child replaces it
        // if it has more than one child
            // step right one
            // then step all the way left and replace the deleted value with the far left one
        bool exist = false;
        ...
        else // this node has two children
        {
            ...
            if (parentNode->left->value == node_deleted->value) // figure out if value being deleted is on the left side of the node
            {
